...
import API_URL from '../config'; // <--- IMPORT CRUCIAL
...
export default function FeedPage() {
  ...
  const saveEdit = async (postId) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/posts/${postId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ content: editContent }) });
          ...
          <div key={post._id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '10px 20px', borderTop: '1px solid #f0f0f0', display:'flex', justifyContent:'space-around', background:'#fafafa', alignItems:'center' }}>
              <div style={{ display:'flex', gap:20 }}>
                <button onClick={() => handleLike(post._id)} style={{ background: 'transparent', color: isLiked ? '#e11d48' : '#666', boxShadow:'none', padding:0, display:'flex', alignItems:'center', gap:5 }}>{isLiked ? <FaHeart /> : <FaRegHeart />} {post.likes.length}</button>
            ...
            {user && (
                <div style={{ padding: '10px 20px', display: 'flex', gap: '10px', borderTop: '1px solid #eee' }}><input placeholder="Commenter..." value={commentText[post._id] || ''} onChange={e => setCommentText({...commentText, [post._id]: e.target.value})} style={{ marginBottom: 0, borderRadius:'20px' }} /><button onClick={() => handleComment(post._id)} style={{ borderRadius:'20px', padding:'5px 15px' }}><FaPaperPlane /></button></div>
            )}
          </div>
        );
      })}
