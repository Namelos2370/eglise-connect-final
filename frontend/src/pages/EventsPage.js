...
export default function EventsPage() {
  ...
  const [selectedImage, setSelectedImage] = useState(null); // État Lightbox
  ...
  const handleRSVP = async (eventId) => {
    ...
    const res = await fetch(`http://localhost:3002/events/${eventId}/rsvp`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleDelete = async (eventId) => {
    ...
    const token = localStorage.getItem('token');
    const res = await fetch(`http://localhost:3002/events/${eventId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleComment = async (eventId) => {
    ...
    if (!text) return;
    const token = localStorage.getItem('token');
    const res = await fetch(`http://localhost:3002/events/${eventId}/comment`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ text }) });
    if (res.ok) {
  ...
  if (loading) return <Spinner />; // Affichage du Spinner

  return (
    <div style={{ padding: '20px', maxWidth:'800px', margin:'0 auto' }}>
      {/* Lightbox pour zoomer */}
        ...
        return (
          <div key={event._id} className="card" style={{ padding:0, overflow:'hidden', border:'none', marginBottom:'25px' }}>
            ...
            <div style={{ padding:'20px' }}>
                ...
                <p style={{ color: '#555', fontSize: '0.9em', lineHeight:'1.6' }}><strong>{new Date(event.date).toLocaleDateString()}</strong> à {event.location}</p>
                ...
                {showComments[event._id] && (
                    <div style={{ background:'#f9f9f9', padding:'15px', marginTop:'15px', borderRadius:'10px' }}>
                        {event.comments && event.comments.map((com, idx) => (
                            <div key={idx} style={{ marginBottom:'10px', fontSize:'0.9em' }}>
                                <strong>{com.author?.name}: </strong> {com.text}
                            </div>
                        ))}
                        {user && (
                            <div style={{ display:'flex', gap:5, marginTop:'10px' }}>
                                <input placeholder="Question ?" value={commentText[event._id] || ''} onChange={e => setCommentText({...commentText, [event._id]: e.target.value})} style={{ margin:0, height:'35px' }} />
                                <button onClick={() => handleComment(event._id)} style={{ width:'40px', padding:0, display:'flex', alignItems:'center', justifyContent:'center' }}><FaPaperPlane /></button>
                            </div>
                        )}
                    </div>
                )}
